// m3-dark.jsx — Dark "carbon + Rolex green + red" theme (ported from m1-lib + m2-lib).
...
const TL = {
  ...
  mut:   '#90a89a',
  dim:   '#566b5e',
  line:  'rgba(110,210,150,0.12)',
  lineS: 'rgba(110,210,150,0.24)',
  blue:  'oklch(66% 0.12 150)',   // bright emerald (secondary)
  blueD: 'oklch(56% 0.12 152)',
  blueWash: 'oklch(26% 0.08 156)',
  clay:  'oklch(60% 0.20 27)',    // racing red (accent)
  clayWash: 'oklch(28% 0.10 27)',
  shadow: '0 14px 44px rgba(0,0,0,0.62)',
  shadowSm: '0 6px 18px rgba(0,0,0,0.5)',
};
...
function SlotL({ label, tag, w = 360, h = 230, accent = TL.blue }) {
  return (
    <div style={{ width: w, height: h, position: 'relative', overflow: 'hidden', background: 'repeating-linear-gradient(125deg, #14211a 0 11px, #0f1712 11px 22px)', border: `1px solid ${TL.lineS}`, borderRadius: 6 }}>
...
function GlobalBGL() {
  const t = useTime();
  const ax = 20 + Math.sin(t * 0.07) * 9, ay = 18 + Math.cos(t * 0.05) * 7;
  const bx = 84 + Math.cos(t * 0.06) * 9, by = 84 + Math.sin(t * 0.045) * 6;
  const gridShift = (t * 4) % 56;
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: `linear-gradient(160deg, ${TL.bg2}, ${TL.bg})` }}>
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(circle 840px at ${ax}% ${ay}%, oklch(46% 0.14 158 / 0.32), transparent 60%)` }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(circle 680px at ${bx}% ${by}%, oklch(48% 0.17 28 / 0.20), transparent 60%)` }} />
      ...
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 120% 120% at 50% 50%, transparent 50%, rgba(0,0,0,0.62) 100%)' }} />
    </div>
  );
}
...
const TEAL  = 'oklch(58% 0.13 158)';
const TEALD = 'oklch(48% 0.13 158)';
const TEALWASH = 'oklch(28% 0.09 158)';
...
function ProcessPipe({ d, start, t, color = TL.blue, dur = 0.8 }) {
  ...
  return (
    <>
      <path d={d} fill="none" stroke="rgba(60,160,110,0.16)" strokeWidth="22" strokeLinecap="round" strokeLinejoin="round" opacity={e} />
...
function ClassShellM2({ scenes, claseNo, claseName }) {
  ...
  const total = acc;
  return (
    <Stage width={1920} height={1080} duration={total} background={TL.bg} persistKey={`m3c${claseNo}`}>
      <div id="video-root" data-screen-label={`M3 Clase ${claseNo}`} style={{ position: 'absolute', inset: 0 }}>
        <GlobalBGL />
        {timed.map((sc, i) => <sc.C key={i} start={sc.start} dur={sc.dur} />)}
...
function ChromeM2({ timed, claseNo, claseName }) {
  ...
  return (
    <div style={{ position: 'absolute', left: 54, bottom: 46, zIndex: 40, fontFamily: MONOL, fontSize: 13, letterSpacing: '0.2em', color: TL.mut, display: 'flex', alignItems: 'center', gap: 14, textTransform: 'uppercase' }}>
      <span style={{ color: TEAL, fontWeight: 600 }}>M3 · CLASE {String(claseNo).padStart(2, '0')}</span>
      <span style={{ width: 30, height: 1, background: TL.lineS }} />
      <span>{claseName}</span>
      {label && <><span style={{ width: 30, height: 1, background: TL.lineS }} /><span style={{ color: TL.dim }}>{label}</span></>}
    </div>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: op }}>
      ...
      <div style={{ position: 'absolute', left: '9%', top: '50%', transform: 'translateY(-50%)', width: 1220 }}>
        <KickerL start={s + 0.4} dur={dur - 0.6} text={`Módulo 3 · Clase ${claseNo}`} x="0" y="-130px" align="left" color={TEAL} />
